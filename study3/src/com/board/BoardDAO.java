...
public class BoardDAO {
	...
	// insertBoard: 답변을 달때의 용도, 게시물올릴때의 용도
	...
	public int maxBoardNum() {
		...
		try {
			sql="SELECT NVL(MAX(boardNum),0) FROM board"; // boardNum의 맥스값(최대값)
	...
	public int dataCount(String searchKey, String searchValue) {
		...
		StringBuffer sb = new StringBuffer();
		try {
			sb.append("SELECT COUNT(*) FROM board b");
			sb.append(" JOIN member1 m ON b.userId = m.userId ");
			sb.append(" WHERE ");
	...
	public BoardDTO readBoard(int boardNum) {
		...
		try {
			...
			sql="select boardNum, userName, subject, content, hitCount, created, depth, ORDERNO, groupnum, parent, b.userid from board b JOIN member1 m ON b.userId = m.userId where boardNum=?";
